...
- **search** — ディレクトリを歩き（vcs / build / 依存 / バイナリは除外）、ファイルを 8 本のアームで
  探す（各アームは必要な文法を 1 回だけコンパイルし、リテラルのパターンは行に割る前にテキスト全体で
  確かめる）。マッチをファイルごとにまとめ、同一行を畳み、上限をかけ、outline で囲んでいるシンボルを付ける。
